...
class Builder
{
   ...
   public function build(?string $html = null,?string $version = null): string {
    ...
    public function buildcss(?string $html = null, ?string $version = null): string {
    
     try {
         $path = BP . '/pub/media/tailwind/';
        ...
        $inputCssFile = $path . 'i.css';
        $defaultCssContent = "@tailwind base;\n@tailwind components;\n@tailwind utilities;\n";
        self::write($inputCssFile, $defaultCssContent);

        $outputFile = $path . 'o.css';
         if (file_exists($outputFile) && !is_writable($outputFile)) {
        chmod($outputFile, 0664); // Adjust file permissions
         }

        self::write($outputFile, null);
